...
### Spotify Hub funcionality
In order to run the Spotify Hub, you must first have a Spotify premium account and send your full name and associated email address to one of the developers (delaney.m@northeastern.edu) so we can add you to our Spotify API app. Sign in must then happen before creating/joining a town. If a device is not selected, the Spotify Hubs will still function but you will not have the playback capability. In order for a device to show up, you must be signed into spotify and have the application open, but it does not need to be the same device you are using to access Cover.Town. A Spotify hub will allow you to create a song queue with others in Covey.Town and play songs at the same time. You also have the ability to search songs, see album covers, add comments, like/dislike comments, clear the queue, save/retrieve songs, see advanced song analytics, and affect the queue ordering by liking/disliking.